...
description: "Bulk create or update person notes from Slack profiles. Given user IDs or names, checks Slack for role/title/team, checks vault for existing notes, creates missing ones, updates stale ones, and updates Pessoas e Contexto index."
...
For each person:
- Check if `equipe/pessoas/<Real Name>.md` exists.
...
For each person without a vault note, create `equipe/pessoas/<Real Name>.md`:
...
### 5. Update Pessoas e Contexto Index

Read `equipe/Pessoas e Contexto.md`. For each new person:
- Add a one-line entry in the `## People` section.
- Format: `- [[Person Name]] — Title. Brief context.`

### 6. Check Team Notes

If a person's team is identifiable and the team doesn't have a note in `equipe/times/`:
...
- Pessoas e Contexto updated: yes/no
