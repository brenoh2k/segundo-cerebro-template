...
1. **Conquistas**: Read `desempenho/Conquistas.md` and the quarterly brag notes in `desempenho/conquistas/` for the specified period. Extract all achievements with their evidence links.

2. **Decisions Led**: Search `trabalho/` for decision records where the user was the owner/driver. Use `qmd query "decision" --json` filtered by date if available, or grep frontmatter for `tags: [decision]` within the date range.

3. **Incidents Handled**: Read all notes in `trabalho/incidentes/` from the period. Extract severity, role played, outcome, and learnings.

4. **Competency Evidence**: Read competency notes in `desempenho/competencias/`. For each competency, search for backlinks from work notes in the period. Use `obsidian backlinks file="<competency>"` or grep.

5. **1-on-1 Feedback**: Read 1-on-1 notes in `trabalho/individuais/` from the period. Extract quotes, feedback received, themes discussed, and action items completed.

6. **PR Evidence**: Read any PR analysis files in `desempenho/evidencias/` for the period.

7. **Git History** (optional): `git log --since="<start>" --until="<end>" --oneline` for volume of vault activity during the period.

## Output

Write the review prep document to `desempenho/<cycle>/Review Prep - <cycle>.md` (e.g., `desempenho/h2-2024/Review Prep - H2 2024.md`) with frontmatter:
