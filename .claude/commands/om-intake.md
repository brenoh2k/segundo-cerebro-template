---
description: "Process all unread meeting notes in trabalho/reunioes/ — reads each file, classifies content, routes to the right vault notes, then clears the inbox."
---

# Meeting Intake

Scans `trabalho/reunioes/` for unprocessed meeting exports and routes everything to the correct vault locations automatically. No arguments needed — just drop files and run.
...
Drop any exported or raw meeting note into `trabalho/reunioes/` first. Run this command after. Naming convention: `YYYY-MM-DD <Topic or Person>.md`.

## Workflow

### 1. Scan the Inbox

List all `.md` files in `trabalho/reunioes/` excluding `README.md`. If the folder is empty, say so and stop.
...
|---|---|
| 1:1 with a specific person | Create `trabalho/individuais/<Person> YYYY-MM-DD.md` using 1:1 note structure |
| Project status update | Append to relevant `trabalho/ativos/<Project>.md` |
| New project or initiative not in vault | Create `trabalho/ativos/<Project>.md` |
| Decision reached | Create Decision Record in `trabalho/` + add to Decisions Log in `trabalho/Index.md` |
| Action item / open task | Append as `- [ ]` to the relevant work note |
| Win or recognition | Add to `desempenho/Conquistas.md` with link to source note |
| New person mentioned not in vault | Create stub in `equipe/pessoas/<Name>.md` |
| Blocker identified | Append to relevant `trabalho/ativos/` note under `## Blockers` or `## Open / Next Steps` |
...
- If a person was mentioned, link to their `equipe/pessoas/` note from the work note and vice versa
