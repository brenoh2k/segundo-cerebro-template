...
Load in order:
1. Person note from `equipe/pessoas/<Name>.md`
2. PR evidence from `desempenho/evidencias/<Name> PRs - <cycle>.md` (run `/om-peer-scan` if missing)
3. Peer review notes from `trabalho/ativos/` (any active review writing notes)
...
Save draft to `rascunhos/<name>-peer-review.md` for copy-pasting.
After submission, promote to `desempenho/<cycle>/Peer Review - <Full Name>.md`.
