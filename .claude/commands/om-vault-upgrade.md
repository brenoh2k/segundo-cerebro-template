...
- `--dry-run` — generate the migration plan without executing it (saves to `rascunhos/`)
...
4. Check for `vault-manifest.json` in the source:
   ...
   - **Not found**: run version fingerprint detection from the target's `vault-manifest.json`:
     - Check for `claude/Memórias.md` without `cerebro/` → v1
     - Check for `cerebro/` + `bases/` without `.claude/agents/` → v2
...
6. Report detection result to the user: "Detected obsidian-mind v2" or "This looks like a PARA-organized Obsidian vault — I'll map Projects → trabalho/ativos, Resources → reference, etc." or "This is a flat vault with no folder structure — I'll classify each note by its content."
...
If `cerebro/Migration Log.md` exists in the target vault, read it and extract the recorded hashes. Each log entry stores both a **source hash** (original file content) and a **final hash** (transformed content written to target).
...
Files found: 87
  ...
  CLASSIFY:  5 uncategorized → rascunhos/migrate-review/
...
Transformations:
  ...
  - 1 v1 monolith Memórias.md → split into topic files
```
...
|---|--------|--------|--------|------------|
| 1 | equipe/pessoas/Jane.md | equipe/pessoas/Jane.md | COPY | +description |
| 2 | cerebro/Memórias.md | cerebro/Memórias.md | REPLACE | — |
| 3 | daily/2026-03-15.md | trabalho/individuais/... | CLASSIFY+COPY | +quarter, +tags |
```
...
**For `--dry-run`**: write the plan to `rascunhos/vault-upgrade-plan-YYYY-MM-DD.md` and stop.
...
Migration complete.
  ...
  ✓ Indexes rebuilt (trabalho/Index.md, Pessoas e Contexto, Conquistas, Memories)
  ✓ .obsidian/ config copied

  ⚠ 3 orphan notes (no inbound links) — run /om-vault-audit to fix
  ⚠ 5 files in rascunhos/migrate-review/ need manual sorting

  Migration log: cerebro/Migration Log.md
```

Offer next steps:
- "Run `/om-vault-audit` for a full structural check"
- "Review `rascunhos/migrate-review/` for uncategorized files"
- "Check `cerebro/Migration Log.md` for the full receipt and CLAUDE.md diff"
...
- **Non-work content is welcome.** Not every note fits trabalho/equipe/perf. Personal journal entries go to `referencia/personal/`, book and course notes go to `referencia/learning/`. The goal is zero data loss, not forcing everything into work categories.
...
- This command creates `rascunhos/migrate-review/` only if there are uncategorized files. Clean it up after manual sorting.
