---
description: "Weekly synthesis — cross-session review of vault activity, Norte alignment, patterns, uncaptured wins, and forward priorities."
---
...
- Read any `trabalho/individuais/*.md` notes from this week
- Check `trabalho/ativos/` for status changes
- Check `trabalho/incidentes/` for new or updated incidents
- Read today's daily note if it exists: `obsidian daily:read`

### 2. Norte Alignment

Read `cerebro/Norte.md` and compare actual activity against stated focus:
...
Additionally check:
- Were completed items logged in `desempenho/Conquistas.md` or the current quarter's brag note?
- Any 1:1 feedback or kudos not captured?
- Incident contributions not bragged about?

### 5. Competency Signal Mapping

For each competency in `desempenho/competencias/`:
...
- Norte goals that need attention next week
...
- **Norte Check**: alignment status — what's on track, what drifted, what's silent
...
- "Should I update Norte with any focus shifts?"
- "Want me to save this as `rascunhos/weekly-YYYY-MM-DD.md`?"
