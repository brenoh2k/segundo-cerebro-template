 ...
 * cerebro/ frontmatter. The caller owns the destination — the memory dir
...
/**
 * Collect cerebro/ notes (name + description) for index generation. Exported
 * so the tidy-fix consumer regenerates the index from the same walk.
 * Returns null when cerebro/ is missing.
 */
export function collectBrainNotes(root: string): BrainNote[] | null {
	let entries: string[] = [];
	try {
		entries = readdirSync(join(root, "cerebro")).filter((n) =>
	...
	return entries.map((f) => {
		...
		try {
			description = extractFrontmatterField(
				readFileSync(join(root, "cerebro", f), "utf-8"),
				"description",
			);
		} catch {
...
function main(): void {
	...
	if (notes === null) {
		process.stderr.write("cerebro/ not found — run from the vault root.\n");
		process.exit(1);
	}
