...
describe("shouldSkipFile — skip rules", () => {
	...
	test("skips modelos/", () => {
		assert.equal(shouldSkipFile("/vault/modelos/Nota de Trabalho.md"), true);
	});
	test("skips rascunhos/", () => {
		assert.equal(shouldSkipFile("/vault/rascunhos/draft.md"), true);
	});
	...
	test("does NOT skip a regular vault note", () => {
		assert.equal(shouldSkipFile("/vault/trabalho/ativos/project.md"), false);
	});
	test("does NOT skip a normal .obsidian-ish name (only the literal segment matters)", () => {
		...
		assert.equal(shouldSkipFile("/vault/trabalho/ativos/.obsidianish.md"), false);
	});
});
...
describe("isBlockedMemoryPath", () => {
	...
	test("requires both .claude and memory segments", () => {
		assert.equal(isBlockedMemoryPath("/vault/cerebro/Memórias.md"), false);
