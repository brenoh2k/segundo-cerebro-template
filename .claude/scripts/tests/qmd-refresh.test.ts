...
describe("shouldRefreshForPath — accepts vault markdown", () => {
	test("accepts a relative vault note", () => {
		assert.equal(shouldRefreshForPath("trabalho/ativos/project.md"), true);
	});
	test("accepts an absolute Unix vault note", () => {
		assert.equal(
			shouldRefreshForPath("/Users/me/vault/trabalho/ativos/project.md"),
			true,
		);
	});
	...
	test("accepts cerebro/, equipe/, desempenho/ paths (vault content roots)", () => {
		assert.equal(shouldRefreshForPath("cerebro/Padrões.md"), true);
		assert.equal(shouldRefreshForPath("equipe/pessoas/Alice.md"), true);
		assert.equal(shouldRefreshForPath("desempenho/Conquistas.md"), true);
	});
	test("accepts paths with spaces (Obsidian-style filenames)", () => {
		assert.equal(shouldRefreshForPath("trabalho/individuais/Jane Smith 2026-04-05.md"), true);
	});
});
...
describe("shouldRefreshForPath — rejects non-markdown writes", () => {
	...
	test("rejects .txt, .json, .ts, .yaml", () => {
		assert.equal(shouldRefreshForPath("trabalho/note.txt"), false);
