...
before(() => {
	...
	note("trabalho/ativos/Done Solo.md", "completed", "2025-06-01");
	note("trabalho/ativos/Live One.md", "active", "2026-07-01");
	note("trabalho/ativos/Mixed Topic/Done.md", "completed", "2026-01-01");
	note("trabalho/ativos/Mixed Topic/Live.md", "active", "2026-01-01");
	note("trabalho/ativos/Full Topic/A.md", "completed", "2024-03-01");
	note("trabalho/ativos/Full Topic/B.md", "completed", "2024-04-01");
	note("trabalho/ativos/Split Years/A.md", "completed", "2023-11-01");
	note("trabalho/ativos/Split Years/B.md", "completed", "2024-02-01");
	mkdirSync(join(ROOT, "cerebro"), { recursive: true });
	writeFileSync(
		join(ROOT, "cerebro/Padrões.md"),
		'---\ndescription: "patterns"\n---\n# P\n',
	);
	writeFileSync(join(ROOT, "cerebro/Collide.md"), "# existing brain note\n");
...
describe("tidy-fix", () => {
	test("dry-run lists both tiers and touches nothing", () => {
		...
		// Nothing moved.
		assert.ok(existsSync(join(ROOT, "trabalho/ativos/Done Solo.md")));
		assert.ok(existsSync(join(MEMDIR, "stray-note.md")));
		assert.ok(!existsSync(join(ROOT, "cerebro/stray-note.md")));
	});
	...
	test("apply acts on the deterministic tier only", () => {
		...
		assert.ok(existsSync(join(ROOT, "trabalho/arquivo/2025/Done Solo.md")));
		assert.ok(!existsSync(join(ROOT, "trabalho/ativos/Done Solo.md")));
		// Fully-completed cluster moves whole.
		assert.ok(existsSync(join(ROOT, "trabalho/arquivo/2024/Full Topic/A.md")));
		assert.ok(!existsSync(join(ROOT, "trabalho/ativos/Full Topic")));
		// Multi-year cluster refused — untouched.
		assert.ok(existsSync(join(ROOT, "trabalho/ativos/Split Years/A.md")));
		// Mixed cluster refused — untouched.
		assert.ok(existsSync(join(ROOT, "trabalho/ativos/Mixed Topic/Done.md")));
		assert.ok(existsSync(join(ROOT, "trabalho/ativos/Mixed Topic/Live.md")));
		// Active note untouched.
		assert.ok(existsSync(join(ROOT, "trabalho/ativos/Live One.md")));
		// Memory stray: copied, verified, removed; index regenerated.
		assert.equal(
			readFileSync(join(ROOT, "cerebro/stray-note.md"), "utf-8"),
		...
		assert.equal(
			readFileSync(join(ROOT, "cerebro/Collide.md"), "utf-8"),
			"# existing brain note\n",
		);
	});
