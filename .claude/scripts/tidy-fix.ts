 ...
 *
 *   ACTS on:
 *     - completed-in-active  → git mv to trabalho/arquivo/YYYY/ (year from the
 ...
 *       stray file into cerebro/, regenerate the MEMORY.md index, VERIFY the
 ...
 *       mixed-completion clusters, cerebro/ name collisions.
 *
 *   Never edits prose indexes (trabalho/Index.md rows are reported, not moved
...
function fixCompletedInActive(
	...
	for (const rel of completed) {
		...
		if (slash === -1) {
			...
			const dest = `trabalho/arquivo/${year}/${inside}`;
			report.fixed.push(`${rel} → ${dest}`);
			if (apply) moveTracked(vaultRoot, rel, dest, report.notes);
			report.notes.push(`  move its row in trabalho/Index.md (${basename(rel)})`);
			continue;
		}
...
function fixMisplacedMemory(
	...
	for (const stray of entries) {
		const target = join(vaultRoot, "cerebro", stray);
		if (existsSync(target)) {
			report.refused.push(
				`memory/${stray} — cerebro/${stray} already exists; merging content is judgment`,
			);
			continue;
		}
		report.fixed.push(`memory/${stray} → cerebro/${stray} (copy, verify, remove)`);
		report.notes.push(
			`  review cerebro/${stray} before committing — auto-memory content can be personal, and cerebro/ is repo-tracked`,
		);
		if (!apply) continue;
		const content = readFileSync(join(memDir, stray), "utf-8");
		mkdirSync(join(vaultRoot, "cerebro"), { recursive: true });
	...
	if (apply && migrated > 0) {
		// Regenerate the index from cerebro/ (now including the migrated notes),
		// THEN remove the verified strays — hegu-1's sequence from #81.
		// Removal is GATED on successful regeneration: if the index step
		// fails, the strays stay (copies remain in cerebro/ for inspection).
		let indexOk = false;
		try {
			...
			if (notes !== null) {
				...
				report.notes.push(
					"  index NOT regenerated (cerebro/ unreadable) — strays left in place",
				);
			}
		} catch {
			report.notes.push(
		...
		for (const stray of entries) {
			...
			try {
				...
				report.notes.push(
					`  could not remove memory/${stray} — copy in cerebro/ is verified; remove by hand`,
				);
			}
		}
	}
...
function main(): void {
	...
	if (scan.inboxPressure !== null) {
		report.refused.push(
			`${scan.inboxPressure.count} raw export(s) in trabalho/reunioes/ — run /om-intake`,
		);
	}
