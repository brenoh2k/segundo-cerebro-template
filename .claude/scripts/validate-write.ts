...
// there — durable knowledge goes to cerebro/ topic notes per CLAUDE.md.
...
if (isBlockedMemoryPath(filePath) || isBlockedMemoryPath(resolvedPath)) {
	...
	const memoryContext = [
		...
		"All durable knowledge belongs in the vault under `cerebro/` topic notes:",
		"",
		"- Patterns/conventions → `cerebro/Padrões.md`",
		"- Things that bit before → `cerebro/Armadilhas.md`",
		"- Architectural/workflow decisions → `cerebro/Decisões-Chave.md`",
		"- Recent context, relationships, tools → `cerebro/Memórias.md`",
		"- New topic → new `cerebro/<Topic>.md` note + index in `cerebro/Memórias.md`",
		"",
...
try {
	...
	if (cluster !== null) {
		...
		policyResults.push({
			...
			suggested_target: "trabalho/ativos/<Topic>/",
			action: "flag",
		});
	}
} catch {
