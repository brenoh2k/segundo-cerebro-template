 ...
 *
 * Responsibility: personalize the managed `cerebro/Norte.md` heading with the
 ...
 * their name blank, and `# Norte — ` would be an eyesore.
 *
...
export default async function personalize(ctx: PersonalizeContext): Promise<void> {
  ...
  // still leave Norte untouched rather than write `# Norte — `.
...
/**
 * Replace `# Norte` with `# Norte — <userName>` once. Idempotent:
 ...
 * if `cerebro/` was deselected (the `cerebro` module is `removable: false`,
...
export async function personalizeNorthStar(vaultRoot: string, userName: string): Promise<void> {
  const target = join(vaultRoot, 'cerebro', 'Norte.md');
  let original: string;
  try {
    original = await readFile(target, 'utf-8');
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
      console.log(`obsidian-mind: ${target} not present — skipping Norte personalization.`);
      return;
    }
    throw err;
  }
  ...
  // (heading reads `# Norte — <something>`), `^# Norte$/m` won't
  // match and the file is left untouched — making the hook idempotent.
  if (!/^# Norte$/m.test(original)) {
    return;
  }

  const personalized = original.replace(/^# Norte$/m, `# Norte — ${userName}`);
  await writeFile(target, personalized, 'utf-8');
  console.log(`obsidian-mind: personalized cerebro/Norte.md for ${userName}`);
}
