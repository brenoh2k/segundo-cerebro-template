  ...
  if (nowMajor >= 7 && !(prevMajor >= 7)) {
    console.log(
      'v7 note: the auto-memory MEMORY.md (~/.claude/projects/<slug>/memory/) is now a DERIVED index — ' +
        'regenerated from cerebro/ frontmatter, and regeneration overwrites hand edits. ' +
        'If yours accumulated hand-written content, migrate it into cerebro/ topic notes before the next wrap-up regenerates the index.',
    );
  }
