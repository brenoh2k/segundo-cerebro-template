...
Vault pessoal -- cérebro externo para reuniões, tarefas, decisões e conhecimento organizacional, junto com o contexto que o Claude usa em sessão. Construído sobre o template [obsidian-mind](https://github.com/breferrari/obsidian-mind); a mecânica abaixo (hooks, convenções de nota, QMD) vem do template e continua valendo.

## Personalize Isto Primeiro

Este vault chega vazio de conteúdo pessoal -- só a estrutura, os hooks, os comandos e as skills. Antes de usar a sério:

1. Leia `cerebro/Norte.md` e preencha seus próprios objetivos, metas de curto/médio/longo prazo e antimetas.
2. Ajuste esta seção (`## Personalize Isto Primeiro` pode virar `## Seu Contexto`) com uma descrição de 2-3 frases do seu papel, seu contexto de trabalho e de onde vêm suas entradas reais (Calendar, ferramenta de tarefas, Slack, etc.) -- veja o exemplo real que motivou esse hábito em [[Padrões]] (padrão de contexto explícito de vault).
3. Se seu trabalho tem um conceito de "conta" ou "cliente" que merece pasta própria (nem todo mundo tem), crie uma pasta de topo nova (ex. `contas/`), um template em `modelos/`, e registre a nova pasta em `vault-manifest.json` (`user_content_roots`, `open_loop_dirs` se fizer sentido) e nesta tabela de Estrutura do Vault.
4. `desempenho/` já vem ativo (brag doc, competências, ciclo de avaliação) -- é o fluxo original do template, pensado para review profissional. Se não fizer sentido para seu contexto, marque como dormente: um comentário aqui no CLAUDE.md dizendo o quê e por quê, nunca apagando os arquivos.

## Response Language

Respond in Portuguese (pt-BR) by default in this vault -- this applies to every slash command and skill (`/om-*` and any others), including their reports, summaries, and section headers, not just freeform chat. A skill's own definition may template its output in English (e.g. "Done / Fixed / Flagged / Suggested" in `/om-wrap-up`) -- treat that as a structural label to follow, not literal text to emit; translate headers and all prose to Portuguese. Leave code, file paths, wikilinks, frontmatter keys, and any header a hook/command parses literally (see the bilingual convention documented in `cerebro/Padrões.md`) unchanged. Switch to English only if the user writes to you in English first.
...
| `bases/` | **All Bases centralized** -- dynamic views for navigation | `Painel de Trabalho` (incl. Stale Actives), `Tocados Recentemente` (recency by real mtime — the answer to "what's most recent", not filename dates), `Incidentes`, `Diretório de Pessoas`, `1-1 History`, `Evidências de Review`, `Mapa de Competências`, `Templates` |
| `trabalho/` | Work notes index | `Índice.md` (detailed MOC) |
| `trabalho/ativos/` | **Current projects only** (1-3 files) | Move here when starting, move to archive when done |
| `trabalho/arquivo/YYYY/` | Completed work organized by year | Grows over time |
| `trabalho/incidentes/` | Incident docs (main note + RCA + deep dive + drafts) | Per-incident grouping |
| `trabalho/individuais/` | 1:1 meeting notes (accumulate weekly) | Named `<Person> YYYY-MM-DD.md` |
| `trabalho/reunioes/` | **Meeting notes inbox** -- staging area for raw exports, processed by `/om-intake` | Drop files, run `/om-intake` |
| `trabalho/diario/` | **Daily TODO notes** -- one checkbox checklist per day, aggregated from active work + open loops, generated by `/om-standup`. Obsidian Daily Notes core plugin points here (`.obsidian/daily-notes.json`) | Named `YYYY-MM-DD.md`, template `modelos/TO DO Diário.md` |
| `desempenho/` | Career/performance review material | `Conquistas.md` (brag doc index), `competencias/`, `evidencias/`, `<cycle>/` |
| `cerebro/` | Claude's operational knowledge | `Memórias.md`, `Decisões-Chave.md`, `Padrões.md`, `Armadilhas.md`, `Habilidades.md`, `Norte.md` |
| `memorias/YYYY/MM/` | **Cross-repo agent memory** -- durable lessons recorded over MCP by sessions working in *other* repositories. Time is the only thing in the path; reach is declared in frontmatter | Browse via `bases/Memórias.base`; never edit by hand |
| `equipe/` | Organizational knowledge index | `Pessoas e Contexto.md` (MOC) |
| `equipe/pessoas/` | Atomic person notes | One note per person |
| `equipe/times/` | Team notes as graph nodes | One note per team |
| `referencia/` | Codebase/domain knowledge, architecture maps | Flow docs, architecture docs |
| `rascunhos/` | Scratchpad for drafts and reasoning | Named `YYYY-MM-DD-topic.md` |
| `modelos/` | Obsidian templates | `Nota de Trabalho.md`, `Registro de Decisão.md`, etc. |
| `.claude/commands/` | Slash commands (auto-surfaced in-session; catalog in `cerebro/Habilidades.md`) | One `.md` per command |
...
> [!tip] Adding a domain-specific top-level folder
> If your work has a first-class entity beyond people/teams/projects (client accounts, research subjects, product lines...), add it as a sibling of `equipe/`, following the same pattern: an index note + `modelos/<Type>.md` template + an entry in `vault-manifest.json`'s `user_content_roots`. See **Personalize Isto Primeiro** above.
...
The `SessionStart` hook automatically injects rich context: vault file listing, Norte goals, active work, recent git changes, open tasks (aggregated from `trabalho/ativos/` and the vault root, excluding infrastructure files), vault-hygiene drift flags (act on them or run `/om-tidy`), and triggers a QMD re-index. Most context is already loaded -- you don't need to manually read files.

**Shortcut**: Run `/om-standup` for a structured morning kickoff that reads everything and presents a summary with suggested priorities. It can also reconcile yesterday's calendar against the vault, if a calendar connector is wired up -- see the command file for the current mechanism.

If doing it manually:

1. Read `Home.md` -- vault entry point with embedded dashboards
2. Read `cerebro/Norte.md` -- ground suggestions in current goals
3. Check `trabalho/Índice.md` -- see active projects and recent notes
4. Scan `cerebro/Memórias.md` -- index of memory topics, then read relevant topic notes
...
1. **Archive completed projects**: `git mv` from `trabalho/ativos/` to `trabalho/arquivo/YYYY/`, update `status: completed` (or use `/om-project-archive`)
2. Update `trabalho/Índice.md` if new notes or decisions were created
3. Update the relevant brain topic note (`cerebro/Decisões-Chave.md`, `cerebro/Padrões.md`, `cerebro/Armadilhas.md`) with key learnings
4. Update `equipe/Pessoas e Contexto.md` if org knowledge changed
5. Update `desempenho/Conquistas.md` if a win happened this session
6. Offer to update `cerebro/Norte.md` if goals shifted or new focus emerged
7. Verify all new notes link to at least one existing note (orphans are bugs)
8. Run `/om-vault-audit` if the session created many notes

Skip steps that don't apply. The goal is transferring durable knowledge from conversation to vault state.

### Thinking Workflow

Use `rascunhos/` for drafts, reasoning, and analysis before writing final notes. **Thinking notes are scratchpads, not storage.** They exist to help you reason -- once the reasoning produces durable knowledge, promote it to proper notes and delete the scratchpad.

1. Create a thinking note: `rascunhos/YYYY-MM-DD-descriptive-name.md`
...
2. **Use templates** from `modelos/`. Fill `{{placeholders}}` with real values.
...
3. **Place files correctly**:
   - **Active** work notes, decisions, peer review prep -- `trabalho/ativos/`
   - **Completed** work notes -- `trabalho/arquivo/YYYY/` (by year)
   - Incident docs -- `trabalho/incidentes/`
   - 1:1 meeting notes -- `trabalho/individuais/`
   - Performance/review content -- `desempenho/`
   - People -- `equipe/pessoas/`
   - Teams -- `equipe/times/`
   - Claude operational context -- `cerebro/`
   - Domain/codebase knowledge -- `referencia/`
   - Drafts -- `rascunhos/`
...
4. **Name files descriptively.** Use the note title as filename. **Date convention:** prefix **point-in-time** notes with a date — 1:1s (`<Person> YYYY-MM-DD.md`), dated captures, meeting exports. **Living and entity notes use a bare title** — work notes, person/team notes, competencies, `cerebro/`, `referencia/`. *Why the split:* a creation-date prefix on a continuously-edited note **inverts the recency signal** (a living doc looks stale while being updated daily). For "what's most recent," use `bases/Tocados Recentemente.base` (real modified time), never filename dates.
5. **Group multi-note workstreams in a subfolder.** Once a workstream has more than one note, it gets a folder (`trabalho/ativos/<Topic>/`; the archive mirrors the grouping). Folders are the lifecycle/context axis only — links remain the primary organization (wikilinks resolve by name across folders, so grouping never breaks links, QMD, or the graph). The hygiene scan and write-time cluster sensor flag candidates; `/om-project-archive` moves whole clusters.

### Note Types

| Type | Location | Naming | Key Sections |
|------|----------|--------|--------------|
| Work note | `trabalho/ativos/` (then `arquivo/YYYY/` when done) | Descriptive title | Context, What/Why, Links, Related |
| Incident | `trabalho/incidentes/` | Ticket number or descriptive title | Context, Root Cause, Timeline, Impact, Analysis, Related |
| 1:1 note | `trabalho/individuais/` | `<Person> YYYY-MM-DD.md` | Key Takeaways, Action Items, Quotes, What to Watch, Related |
| Daily TODO note | `trabalho/diario/` | `YYYY-MM-DD.md` | To Do (checkboxes, generated by `/om-standup`), Notas do Dia, Related |
| Person note | `equipe/pessoas/` | Full name | Role & Team, Relationship, Key Moments, Notes |
| Team note | `equipe/times/` | Team name | Members, Scope, Interactions |
| Brain note | `cerebro/` | Topic name | Topic-specific content |
...
- **Index nodes** (Índice, Conquistas, Memórias, Pessoas e Contexto): actively curate links -- they're navigational
- **Person nodes** (equipe/pessoas/): link to projects, teams, evidence. Receive backlinks from work notes.
...
- **Conquistas -> Work note**: every entry links to evidence
- **Memories -> Source**: every memory links to where it was learned
- **Índice -> Everything**: `trabalho/Índice.md` links to all work notes
- **Norte -> Projects**: active focus areas link to project work notes

### Maintaining Indexes

Update these when creating or archiving notes:

- **`trabalho/Índice.md`** -- add to Active Projects or Recent Notes, move completed to Archive
- **`cerebro/Memórias.md`** -- index of memory topics. Add new memories to the relevant topic note, not here.
- **`cerebro/Habilidades.md`** -- register vault-specific workflows and slash commands
- **`equipe/Pessoas e Contexto.md`** -- update when people, teams, or org structure changes
- **`desempenho/Conquistas.md`** -- update when a win or piece of evidence happens

### Decision Records

1. Create in `trabalho/` using the Decision Record template
2. Link from the work note(s) that led to the decision
3. Add to the Decisions Log table in `trabalho/Índice.md`
4. If significant, note in `cerebro/Decisões-Chave.md`

### Wins & Achievements

Log wins immediately in `desempenho/Conquistas.md` (or the relevant quarter note) with links to evidence -- don't rely on remembering them at review time.

## Norte

`cerebro/Norte.md` is a living document of goals and focus areas.
...
- **Type**: `work-note`, `decision`, `rascunhos`, `north-star`, `person`, `team`, `cerebro`
- **Index**: `index`, `moc`
- **Status** (frontmatter field): `active`, `completed`, `archived`, `proposed`, `accepted`, `deprecated`
- **Team** (frontmatter field on people + work notes): your team names, e.g. `team/platform`
...
- `team: platform` -- find all notes related to a team
- `status: active` -- find active projects
- `quarter: Q1-2026` -- find all work for a quarter (used by Painel de Trabalho Base)
...
| **Vault memories** | `cerebro/` topic notes | Git-tracked, Obsidian-browsable, linked. All durable knowledge lives here. |
| **Cross-repo memories** | `memorias/YYYY/MM/` | Written by sessions in *other* repos over MCP, under an enforced epistemic contract. See below. |

**Which one, when you are working inside the vault:** `cerebro/`. Always. The `memorias/` tree is written by the `om` MCP server on behalf of sessions elsewhere, and the server **refuses** a write from a session running inside the vault — a memory recorded here would be scoped to the vault-as-a-project and reach only the sessions that already read every note directly. Write the note normally and let the hooks file it.

When asked to "remember" something:
1. Find or create the appropriate `cerebro/` topic note (Gotchas, Patterns, Decisões-Chave, etc.)
2. Add the knowledge there with a wikilink to context
3. Update `cerebro/Memórias.md` index if a new topic note was created
...
The SessionStart hook injects a **Brain Topics (read on demand)** index listing each `cerebro/` topic note with its description and an `(empty)` marker for stub notes. Treat that index as a menu:

- When the user's message touches a topic from the index (debugging → Gotchas, "how do we usually…" → Patterns, "why did we decide" → Decisões-Chave, "which command / slash" → Skills), query QMD **first** before answering — call `mcp__qmd__query` with a `query` argument describing the topic (or fall back to `qmd --index <name> query "<topic>"` if MCP is unavailable). The search covers the whole vault, so filter or prioritize results whose `file` path is under `cerebro/`. Do not assume the topic name alone scopes the search.
- If QMD is unavailable, read the specific `cerebro/` note directly with the Read tool. Don't load all of `cerebro/` — only the one(s) matching the topic.
...
Your vault, your notes, your session. The default is the vault's own `user_content_roots`, at the granularity it declares them (`trabalho/ativos/`, not all of `trabalho/`). Set `mcp_exposed_roots` only if this vault holds material that is **not yours to share** — employer-confidential notes, a client's data.
...
| `memory_root` | `memorias` | where cross-repo memories live |
| `mcp_inbox` | `inbox` | fallback destination for `record_work` |

`memorias/` is never served as an ordinary note whatever the config says — memories carry their own scope, and the note surface would bypass it. A note tagged `private` in frontmatter is never served either.

That holds on **every** surface, including the one that reads across both. `recall` serves a promoted block out of `cerebro/` when a capture's `promoted:` marker carries an anchor (`promoted: "cerebro/Gotchas - Engineering#^om-a1b2c3"`), so a foreign repo gets the corrected version rather than the capture as first written. It asks the same exposure predicate as everything else, so a withheld or `private` note is refused there too, and a marker with no anchor still serves nothing. Write anchors with `/om-tidy` — the block you point at is what a session in another repo reads.
...
- **Writing about a person?** -- `equipe/pessoas/`
- **Writing about a team?** -- `equipe/times/`
- **Writing about how the codebase/domain works?** -- `cerebro/` (Patterns, Gotchas, Decisões-Chave) or `referencia/`
- **Writing about what Claude should remember?** -- `cerebro/Memórias.md` topic notes
- **Capturing a 1:1 meeting?** -- `trabalho/individuais/`
- **Tracking active project work?** -- `trabalho/ativos/`
- **Capturing an incident?** -- `trabalho/incidentes/` (use `/om-incident-capture`)
- **Dumping unstructured info?** -- use `/om-dump` to auto-classify and route everything

### Don't Mix Contexts

When capturing meeting notes, task-tracker updates, or team conversations:
- **Project/work evidence** (decisions, deliverables, status) -- vai para a nota de trabalho relevante em `trabalho/`
- **Dinâmica de pessoas** (feedback, papel, relacionamento) -- vai para `equipe/pessoas/`
- **Review/performance evidence** -- vai para `desempenho/`
...
> [!note] Slack/GitHub-flavored commands assume those tools
> `slack-archaeologist`, `people-profiler`, `om-slack-scan`, `om-peer-scan`, `om-incident-capture`, and the `desempenho/` review workflow (`om-self-review`, `om-review-brief`, `om-review-peer`) assume Slack and/or GitHub as your team's tools. If your work doesn't involve those, mark them dormant here (comment, don't delete) the same way you'd document any other convention adjustment for this vault.
...
| Hook | When | What |
|------|------|------|
| SessionStart | On startup/resume | QMD re-index + self-heal, inject Norte, active work, recent changes, tasks, file listing (folders past a note-count threshold collapse to one line), vault-hygiene drift flags (completed-not-archived, ungrouped clusters, 25KB oversize, stale open loops, meetings-inbox pressure); the whole injection is held under a byte budget — over it, low-priority sections degrade to pointers and the closing injection-size meter names each one it dropped |
...
| PreCompact | Before context compaction | Backs up session transcript to `rascunhos/session-logs/` |
...
- **For "what's most recent," use `bases/Tocados Recentemente.base` (real modified time), never filename dates** — a creation-date prefix on a continuously-edited note inverts the recency signal.
...
- When asked to "remember" something, write to the relevant `cerebro/` topic note with a link to context. Never create memory files in `~/.claude/` -- they are not git-tracked.
